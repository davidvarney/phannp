...
// Note: the SDK appends the API key to the request as a query parameter.
// For POST/PUT/DELETE requests the api_key will be present in the request URI
// query (Guzzle's 'query' option) rather than embedded in the form body.
// Inspect the URI query for the api_key and the body for other params.
$this->assertStringContainsString('api_key=test_api_key', $request->getUri()->getQuery());

// Inspect form body for other parameters
...
> For a full changelog see [CHANGELOG.md](./CHANGELOG.md) in the project root.
...
- 2025-10-20: API key handling change
    ...
    - Migration note: If your code previously relied on the SDK placing the API key inside request bodies or using an `auth` array, update any tests or integrations to look for `api_key` in the request query string. This change avoids surprises when endpoints include existing query strings and keeps parameter handling consistent across request methods.

- 2025-10-20: Postcards resource update
    ...
    - Migration note: Update any code or tests that previously called `create('4x6')` to pass `create(['size' => '4x6'])` instead. This validation helps prevent ambiguous calls and makes future postcard options extensible.
